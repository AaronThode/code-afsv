...
[s_w,spectro_w,time_w, freq_w, Fe_w]=warp_transform(hilbert(y.'),r_guess,c1,Fs, Nfft, N_window,N_window_w,n_limit,tsweep,fstart,fend);


%MM=size(spectro_w);
% spectro_stack=zeros(MM(1),MM(2),length(Nchan));
% %sw_stack=zeros(length(Nchan),length(s_w));
% for I=Nchan
%     fprintf('%i of %i channels\n',I,length(Nchan));
%     y=filter(B,1,x(I,:)-mean(x(I,:)));
%     [~,tmp]=warp_transform(hilbert(y.'),r_guess,c1,Fs, Nfft, N_window,N_window_w,n_limit,tsweep,fstart,fend);
%     spectro_stack(:,:,I)=tmp;
%     pause(1);
%     close all
% end
...
%warp_transform.m
function [s_w,spectro_w,time_w, freq_w, Fe_w]= warp_transform(s_ok,r_guess,c1,Fe, Nfft, N_window,N_window_w,t_trim, ...
    t_sweep,fstart,fend) % You need a long window to see the warped modes)
...
%  t_trim: number of point to remove from time series after source deconvolution estimate
%  fstart: vector of start frequencies of piecemeal linear FM sweeps
%  fend: vector of end frequencies of pieceal FM sweeps
%  t_sweep: duration of these sweeps
...
figure
imagesc(time, freq, abs(tfr_sig_rec).^2)
axis xy
ylim([0 Fe/2])
xlabel('Time (sec)')
ylabel('Frequency (Hz)')
...
for Iguess=1:length(r_guess)
    ...
    title(sprintf('Warped signal assuming range %6.2f',r_guess(Iguess)))
...
if 1==0
    ...
    mode=iwarp_temp(mode_temp_warp,Fe_w,r_guess,c1,Fe,N_ok);
    
    
    %% Verification
    
    % you can compute dispersion curve using reallocated spectrogram
    
    [mode_rtf, mode_rtfr,hat]=tfrrsp(hilbert(mode),1:N_ok,Nfft,hamming(N_window));
    [tm,D2]=momftfr(mode_rtfr,1,N_ok,time);
    ...
    hold on
    plot(tm,freq, 'linewidth',2,'color','red')
    
    % You have to restrict the dispersion curve to where it is relevant. You can
    % do that by hand, or by looking at the energy of the mode (either on the
    % original spectrogram or on the filtered mode in time domain)
end
