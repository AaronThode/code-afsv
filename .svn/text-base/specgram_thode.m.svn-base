function [yo,fo,to] = specgram_thode(varargin)
...
if (length(nfft)==1) || use_chirp
    ...
    if ~use_chirp     % USE FFT
        ...
        error('use Nfft size that is a power of 2: 64, 128, 256, 1024, 2048, etc..')
        f = nfft(:);
        f1 = f(1);
        f2 = f(end);
        m = length(f);
     ...
     error('Nfft has to be a single number, not a vector...')
    f = nfft(:);
    q = nwind - noverlap;
...
if length(P) > 3 && ~isempty(P{4})
    ...
    if length(nfft) == 1
        window = .5*(1 - cos(2*pi*(1:nfft)/(nfft+1)));
    else
