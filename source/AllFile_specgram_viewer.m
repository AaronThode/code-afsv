...
elseif Iwant==1
    ...
    if ~exist('tmp','var') || ~isfield(tmp,'N_window')
        ...
        prompt={'Range guess(m)','water speed (m/s)','bottom speed (m/s)','Nfft','N_window', ...
            'Decimation factor:','Number of FM contour points:','Deconvolve? (yes=1)','Beta transform? (no=0, yes= beta value','# mode guess'};
        def={'9000','1490','1650',Nfft,WindowSize,num2str(Rest),'2',deconv_chc,'0','3'};
...
%%Work done with Cedric on extracting array tilt and mode shape

%extract_tilt_and_mode_shape;
