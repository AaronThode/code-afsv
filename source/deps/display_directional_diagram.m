...
if strcmpi(handles.display_view,'Directionality')
    ...
    if strcmpi(handles.display_view,'ItoERatio')
        c_eff=intensity./energy_density;
        imagesc(TT,FF/1000,c_eff);caxis([0 1])
        titstr='Intensity/Energy Ratio';
    elseif strcmpi(handles.display_view,'KEtoPERatio')
        ratioo=normalized_velocity_autospectrum./pressure_autospectrum;
        imagesc(TT,FF/1000,10*log10(ratioo));
        titstr='Kinetic/Potential Ratio';
        %caxis([0 90])
        caxis([-20 20])
    elseif strcmpi(handles.display_view,'IntensityPhase')
        ratioo=atan2d(sqrt(imag(vx).^2+imag(vy).^2),sqrt((real(vx)).^2+(real(vy)).^2));
        imagesc(TT,FF/1000,ratioo);
        titstr='Kinetic/Potential Ratio';
        caxis([0 90])
