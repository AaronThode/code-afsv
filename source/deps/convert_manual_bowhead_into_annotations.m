%function success_flag=convert_manual_bowhead_into_annotations(list_names,filter_params,station_position)
%function success_flag=convert_automated_bowhead_into_annotations(list_names,filter_params,station_position)
%  Creates an annotation event from a TSV file
...
try
    ...
    [Istation,DASAR_list,ltr]=getSiteInfo(Isite,year);
    loc_dir='/Users/thode/Projects/Greeneridge_bowhead_detection/SoundSourceDistributions/ManualAnalysisFiles.dir/Shell_History_Revised';
    
    list_names=sprintf('%s/fixed_rms_AllSites_%i_manual/fixed_rms_AllSite%i_%i_manual.txt',loc_dir,year,Isite,year);
    [data.ind,data.localized]=read_tsv(list_names,0,Inf,DASAR_list);
    ...
    Defaults.Description{Nd+1}='Bowhead Manual Fields';
    ...
    for J=1:Ns
        annotation_names{J}	=	sprintf('%s-notes-BowheadManual-%s.mat',DASAR_list{J},keyword);
        Data_all{J}				=	Defaults;
