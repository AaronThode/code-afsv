...
while II<=length(names)
    ...
    %%%Skip files that we are not allowed to edit
    switch(names{II})
        case {'bearing', 'range','position'}
            %%%Exit switch
        otherwise
            
            Prompt(JJ,:) = {Description{II}, names{II},[]};
            JJ=JJ+1;
            ...
            switch(names{II})
                ...
                case 'link_names'
                    ...
                    %Formats(8,1).items = {'*.itm','Auction Item File';'*.*','All Files'};
                ...
                case 'link_hashtags'
                    ...
                    Formats(9,1).size = [-1 80];
                    %Formats(8,1).items = {'*.itm','Auction Item File';'*.*','All Files'};
                    Formats(9,1).span = [1 2];  % item is 1 field x 3 fields
                ...
                case 'comments'  %%May  be the last field
                    ...
                    %Formats(Irow,1).items = {'*.itm','Auction Item File';'*.*','All Files'};
