...
%%%Assign bottom properties...M8_SSP
%%Check to see if property embedded in name
if ~isempty(strfind(lower(case_bottom),'pekeris'))&&~isnan(str2double(case_bottom((end-3):end)))
    ...
    if cmedtop(2)==1672
        ...
        rho=[1 1.34];  %From 2010 pekeris inversions, average of two closest ranges
    ...
    switch case_bottom
        ...
        case 'sand_shallow'
            ...
            %rms_roughness=1.0;
        case 'M8_inversion' %derived from R_360s_tiltm6_105013start_airgunonly.jpg
            
            cmedtop=[1475 1466 1805 2621 4007];
            cmedbot=[1435 1805 1805 3156 0];
            rho=[1 1.23 1.23 1.23  2.16];
            %pwavatten=[0 0.2 0.5]; %dB/lamb
            %  rho=[1 1.9 3.0];
            pwavatten=[0 0.147 0.147 0.147 0.817]; %dB/lamb
            lthick=[D 50 10  10];
        case {'UpwardRefacting_Multilayer','sand_Arctic_inversion'} %derived from R_360s_tiltm6_105013start_airgunonly.jpg
...
if isnumber
    ...
    switch case_ssp
        ...
        case {'arcticSSP','UpwardRefacting_Multilayer'}
