function [x_deconv,source] = sourceDeconv_dB(x,N,NFFT,Nwindow,y_min,y_max)
...
fig=imagescFun(1:N,(1:NFFT)/NFFT,10*log10(sig_whale),[y_min y_max],'Choose the points to interpolate the 1st mode');
...
t_interp=linspace(t_points(1),t_points(end),round(1+t_points(end)-t_points(1)));
iflaw=interp1(t_points,f_points,t_interp,'linear')';

% Deconvolution (source phase cancelation)
x_f=fft(x,N);
source=fmodany(iflaw);
