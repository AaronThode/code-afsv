%%%master_movie_spike_removal.m%%
function master_movie_spike_removal.m
Isite=0;

param.btol=10;  %bearing tolerance in degrees.  If less than one, accept all bearings
param.min_range=3;  %minimum range from center of array in km
param.o_tol=10; %orientation tolerance in degrees relative to zero
dirpath=sprintf('../Processed.dir/Site_0%i/Shell12_Site%i_InitialRun/morph/', ...
    Isite,Isite);
%dirpath='.';
...
%%If Site 0, change index to 6 to access location data
if Isite==0
    Isite=6;
end

for Ifile=12:length(fnames)
%for Ifile=1:
