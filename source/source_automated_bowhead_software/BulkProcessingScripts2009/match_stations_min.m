%ofunction [match_index,dt,best_mismatch]=match_stations_min(station_anchor,station_other,params,debug_params,fname_anchor,fname_other);
%
% Input:
...
%       Coarse estimate
%    dt_xcorr: more precise dt estimate using cross-correlation
%    best_mismatch:  feature error of match at Icall...


function [match_index,dt,dt_xcorr,best_mismatch]=match_stations_min(station_anchor,station_other,params,debug_params,fname_anchor,fname_other,Ncalls)
...
if exist('debug_params')&&debug_params.cross_channel>1,
    ...
    Ncalls=min([Ncalls length(station_anchor.ctime_min)]);
    %Ncalls=1000;
end
...
% A variable that logs the results of previous xcorr between candidates
%%March through each call on the anchor station
%for Icall=I_anchor_start:Ncalls
for Icall=I_anchor_start:Ncalls
    %Icall=match_index(I,1);
    if rem(Icall, 1000)==0, disp(sprintf('%6.2f percent complete...',round(100*Icall/Ncalls)));end
    ...
    %Reciprical test:  Does preferred candidate call match another anchor call better?
        ...
        for K=1:length(Icandidate)
            ...
            for L=1:length(Icandidate2)
                ...
                elseif (scratchpad(Icandidate2(L),Icandidate(K))==0)
                   % disp(sprintf('Testing countermatch of %i against %i',Icall,Icandidate(K)));
            ...
            % minval is the best match result between a candidate and an anchor call
            [minval,Ival]=min(tol_test2);
            
            %%Check that best candidate match does not match other anchor calls any better
...
%%Images may be segmented, so compare with each ridge segment of the other image...
for I=1:Nsegs,
    ...
    for J=1:length(Icandidate),
        ...
        for K=1:Nsegs_other
            ...
            [dist_min(K),dN(K),dist(K),max_ovlap]=correlate_image(anchor_subimage, other_subimage,Iplot);
            ...
            if Iplot==1;
                ...
                title(sprintf('min corr: %8.4f, standard corr: %8.4f max_ovlap %i',dist_min(K),dist(K),max_ovlap));
                pause

            end
        end
        [tol_test(I,J),Imin]=min(dist_min);
        dN_cand(I,J)=dN(Imin);
...
function [HHmin,dN,HH,max_ovlap]=correlate_image(Y, X,Iplot)

HH=Inf;dN=0;HHmin=Inf;max_ovlap=[];
X=double(X); Y=double(Y);
M=max([size(X,2) size(Y,2)]);

%%Find rows that have no image
...
%Igood=find(sum([X Y],2)>0);
% total_xcorr=zeros(1,2*M+1);
% 
% tic
% for Irows=1:length(Igood)
%    [tmp,lags]=xcorr(X(Igood(Irows),:),Y(Igood(Irows),:),M);
%    total_xcorr=total_xcorr+tmp;
% end
% toc

%[max_ovlap,Ioffset]=max(total_xcorr);

XX=[X(Igood,:) zeros(length(Igood),M-size(X,2))]'; %Each column is now a row of the original image
YY=[Y(Igood,:) zeros(length(Igood),M-size(Y,2))]';
Xmag=sum(sum(X));  %Number of pixels in X
Ymag=sum(sum(Y));  %Number of pixels in Y

Irevx=ones(size(XX,1),1)*((-1).^(1:length(Igood)));  %Enables 1-D cross-correlation, assign alternating sign to
                                                    % each image column (which were originally the rows)
...
% imagesc(Y);
% subplot(2,1,2);
% plot(lags,tmp2+10,'k',lags,total_xcorr,'r');pause;close

max_pixels=sqrt(Xmag.*Ymag);
HH=1-(max_ovlap/max_pixels);
max_pixels=min([Xmag Ymag]);
HHmin=1-(max_ovlap/max_pixels);
%if HHmin<eps HHmin=Inf;end
end
