...
if (numElements == 0)
  perf = 0;
    ...
    perfi=(d).*log((deps+y)./(deps+d))+(1-d).*log((deps+1-y)./(deps+1-d));  %Note, be careful with eps placements
  perf = -(sum(sum(perfi)));
 perf=perf/numElements;  %This step needed because numElements will vary
  %if perf<0.5661
  %    keyboard
  %end
end
...
if (numElements == 0)
  ...
  d = -(y-dd)./((y).*(1-y));  %%%%Note the minus sign is different than Bishop..  suspect definition of e different.
  d=d/numElements;
  d(dontcares) = 0;
  %close all;hist(d);pause;
end
