...
%   C = XCORR(A,B,filtopt), where A and B are length M vectors (M>1), returns
...
%   column vector.
%
%   filtopt: a structure containing 
%           .fmin,.fmax,.Fs: min,max frequencies desired and sampling rate,
%           all in Hz
%
...
%   $Revision: 1.16.4.3 $  $Date: 2006/10/18 03:23:22 $ 
%
%   References:
...
[x,nshift] = shiftdim(x);
[xIsMatrix,autoFlag,maxlag,scaleType,filtopt,msg] = parseinput(x,varargin{:});
error(msg);


if xIsMatrix,
	[c,M,N] = matrixCorr(x,filtopt);
else
	[c,M,N] = vectorXcorr(x,autoFlag,filtopt,varargin{:});
end
...
%----------------------------------------------------------------
function [c,M,N] = matrixCorr(x,filtopt)
% Compute all possible auto- and cross-correlations for a matrix input

[M,N] = size(x);
Nfft=2^nextpow2(2*M-1);
X = fft(x,Nfft);
Imin=floor(filtopt.fmin*Nfft/filtopt.Fs);
Imax=floor(filtopt.fmax*Nfft/filtopt.Fs);
Icon=Nfft-(Imax:-1:Imin)+2;
Ibad=setdiff(1:Nfft,[Imin:Imax Icon]);
X(Ibad,:)=0;
...
%----------------------------------------------------------------
function [c,M,N] = vectorXcorr(x,autoFlag,filtopt,varargin)
...
%----------------------------------------------------------------
function [xIsMatrix,autoFlag,maxlag,scaleType,filtopt,msg] = parseinput(x,varargin)
...
switch nargin,
 case 2,
        filtopt=varargin{1};
case 3,
	% Can be (x,y,filtopt), (x,maxlag,filtopt), or (x,scaleType,filtopt)
    filtopt=varargin{2};
...
case 4,
	% Can be (x,y,maxlag,filtopt), (x,maxlag,scaleType,filtopt) or (x,y,scaleType,filtopt)
...
case 5,
	% Must be (x,y,maxlag,scaleType,filtopt)
	autoFlag = 0;
	y = varargin{1};
