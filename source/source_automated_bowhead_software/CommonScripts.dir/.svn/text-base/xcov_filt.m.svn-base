function [xycov,lags] = xcov(x,y,option1,option2,filtopt)
...
mx = size(x, 1);
if nargin == 2
    [xycov,l] = xcorr_filt(x-ones(mx,1)*mean(x),y);
elseif nargin == 3
    if ischar(y)||(~ischar(y)&&length(y)==1)
        [xycov,l] = xcorr_filt(x-ones(mx,1)*mean(x),y,option1);
    else
        my = size(y, 1);
        [xycov,l] = xcorr_filt(x-ones(mx,1)*mean(x),y-ones(my,1)*mean(y),option1);
    end
elseif nargin == 4
    my = size(y, 1);
    if length(y)==1
        [xycov,l] = xcorr_filt(x-ones(mx,1)*mean(x),y,option1,option2);
    else
        [xycov,l] = xcorr_filt(x-ones(mx,1)*mean(x),y-ones(my,1)*mean(y),option1,option2);
    end
elseif nargin == 5
    my = size(y, 1);
    [xycov,l] = xcorr_filt(x-ones(mx,1)*mean(x),y-ones(my,1)*mean(y),...
                         option1,option2,filtopt);
end
if nargout > 1
    lags = l;
end
