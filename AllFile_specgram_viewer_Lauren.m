...
% Last Modified by GUIDE v2.5 07-Jun-2011 11:18:41
...
if strcmp(lower(handles.filetype),'mdat')|strcmp(lower(handles.filetype),'wav')
...
if tlen<=240
    ...
    slider_step(1)=datenum(0,0,0,0,0,60);
...
%set(hObject,'sliderstep',[1/600 10/600]);
%set(hObject,'sliderstep',[1/dT_min 10/dT_min]);
...
wavwrite(x/(1.1*max(abs(x))),Fs,save_name);
...
if strcmp(handles.filetype,'MDAT')
    ...
    if mychc==2
        ...
        Iplot=0;
        %[x,t,Fs,tstart,junk1,head]=load_data(handles.filetype,handles.min_time , tdate_start,tlen,'all',handles.mydir,handles.myfile);
        [x,t,Fs,tstart,junk1,head]=load_data(handles.filetype,handles.min_time , tdate_start,tlen,'all',handles.mydir,handles.myfile);
        ...
        for Ichan=1:head.Nchan
            ...
            text(0.1,0.9,num2str(Ichan),'color','k','units','norm','fontsize',14,'fontweight','bold');
            colorbar('East');
...
if ~strcmp(handles.display_view,'New Fig')
    ...
    figure(gcf);
    chcc=get(0,'child');
    figchc=menu('Select a figure number:',chcc(2:end));
    figure(figchc);
end
tdate_start=datenum(get(handles.edit_datestr,'String'));
tlen=str2num(get(handles.edit_windowlength,'String'));
...
disp(tmp(:,1)'-tmp(1,1));
disp('Time between points in seconds:');
disp(diff(tmp(:,1)'));
disp('Frequency between points in Hz:');
disp(1000*diff(tmp(:,2)'));
disp('Mean Time between points in seconds:');
disp(mean(diff(tmp(:,1)')));
end
...
switch filetype
    ...
    case 'DAT'
        ...
        Fs=fs;
        [x,tmin,tmax]=read_dat_file(mydir,myfile,Fs,tdate_start,tlen,0);
        head.Nchan=1;
    ...
    case 'WAV'
        ...
        sens0=180; %dB re 1 unit of wav entry
...
mydir=pwd;
Ichan='all';  %Hardwire first channel
[x,t,Fs,tstart,junk,head]=load_data(handles.filetype,handles.min_time , tdate_start,tlen,Ichan,handles.mydir,handles.myfile);
x=x.';

%%For MDAT file, make shallowest element channel 1, so flip
x=fliplr(x);
...
if Ichc==1
    Igood=find(Ksout.fcount>0);
    Ksout.fcount=Ksout.fcount(Igood);
    Ksout.fpower=Ksout.fpower(Igood);
    Ksout.freq=Ksout.freq(Igood);
    Ksout.Kstot=Ksout.Kstot(:,:,Igood);
    Ksout.pgoal=Ksout.pgoal(:,Igood);
    Ksout.Nfft=Nfft;
    Ksout.ovlap=ovlap;
    
    write_covmat(Ksout.freq,Ksout.Kstot,rd,srcname,sprintf('Nfft: %i ovlap %6.2f chann: %s',Nfft,ovlap,mat2str(chann)));
    
    figure
    imagesc(Ksout.freq,[],abs(Ksout.pgoal));
    xlabel('frequency (Hz)')
...
function [Ksout,Ns]=extractKsbest_contour(x,ovlap,Nfft,chann, frange,fr,fbad,Fs,M,keep_zeros,nowin)
...
for I=Isnap
    ...
    if length(findex)<MAXF&&threshold>=10*log10(abs(power(I+1)))
        for J=1:Nf
            ...
            tiltvec=exp(1i*2*pi*f(J)*sintilt*tiltdata.rd);
...
tlen=str2num(get(handles.edit_windowlength,'String'));
%for Ichan=1:8
[x,t,Fs,tstart,junk,head]=load_data(handles.filetype,handles.min_time , tdate_start,tlen,'all',handles.mydir,handles.myfile);
if size(x,2)>1
    x=x';
end
%xall{Ichan}=x;
...
for I=1:2
    ...
    if I==1
        ...
        caxis([80 120])
        title(sprintf('Bslice 2 %s: %s plus %6.2f sec',get(handles.text_filename,'String'),datestr(tdate_start),twant));
    else
        imagesc(FF,head.rd,abs(Bslice2'))
        %caxis([1e9 1e11])
    end
...
for I=1:2
    subplot(2,1,I)
    if I==1
        contourf(FF,head.rd,10*log10(Bslice'), 80:5:120, 'linestyle','none');axis('ij')
        caxis([80 120])
        title(sprintf('%s: %s plus %6.2f sec',get(handles.text_filename,'String'),datestr(tdate_start),twant));
    else
        imagesc(FF,head.rd,(Bslice'))
...
plot(head.rd,mode);
subplot(2,1,2)
angg=180*angle(Bslice2(Ibest,:))/pi;
plot(head.rd,angg);grid on;
